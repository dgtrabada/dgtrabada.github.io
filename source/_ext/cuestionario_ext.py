...
JS = """
   <script>
   // baraja las opciones de cada pregunta al cargar la página
   document.addEventListener('DOMContentLoaded', function () {
     document.querySelectorAll('.qp-opts:not([data-qp-mezclada])').forEach(function (opts) {
       opts.dataset.qpMezclada = '1';
       var ops = Array.from(opts.children);
       for (var i = ops.length - 1; i > 0; i--) {
         var j = Math.floor(Math.random() * (i + 1));
         var t = ops[i]; ops[i] = ops[j]; ops[j] = t;
       }
       ops.forEach(function (l) { opts.appendChild(l); });
     });
   });
   function qpNorm(s) {
   ...
   function qpSolucion(btn) {
     ...
     card.querySelectorAll('.qp-item').forEach(function (it) {
       it.querySelectorAll('.qp-opt').forEach(function (l) { l.className = 'qp-opt'; });
       var correcto = it.querySelector('input[value="' + it.dataset.a + '"]');
